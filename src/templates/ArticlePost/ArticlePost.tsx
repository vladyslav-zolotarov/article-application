...
const ArticlePost = ({ post, showMenu }: ArticlePostProps) => {
  return (
    <ArticleWrapper
      classList={
        "article__item p-5 border rounded-lg shadow bg-white border-gray-200 dark:bg-gray-800 dark:border-gray-700"
      }
    >
      <>
        ...
        <div className="article__item_content mt-5">
          <Link to={`/post/${post?._id}`}>
            <ArticleImage post={post} classList={"mb-6 h-52 w-full"} />
            <ArticleTitle
              post={post}
              classList={
                "block mb-6 text-2xl font-bold tracking-tight text-gray-900 dark:text-white"
              }
              classListSkeleton={
          ...
          <ArticleText
            post={post}
            classList={
              "mb-3 line-clamp-2 block font-normal text-gray-700 dark:text-gray-400"
            }
          />
