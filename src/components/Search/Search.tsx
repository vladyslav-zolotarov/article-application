            ...
            return (
                <li className='py-2 border-b border-gray-200 dark:border-gray-600' key={item._id}>
                    <Link className='flex items-center' to={`/post/${item._id}`}>
                        ...
                        <ArticleViewCount post={item} classList={'flex w-16 items-center'} />
                    </Link>
                </li>
            )
        }
    ...
    return (
        <form className='search_form'>
            <div className="relative w-full">
                <input type="search" onChange={(e) => onHadleFilter(e)} id="search-dropdown" className="block p-2.5 w-96 z-20 text-sm text-gray-900 bg-gray-50 rounded-lg border-gray-100 border-2 focus:ring-blue-500 focus:border-blue-500 dark:bg-gray-700 dark:border-l-gray-700  dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:border-blue-500" placeholder="Search article..." required />
                ...
                {inputValue &&
                    <div className='search_result absolute z-10 mt-2 bg-white rounded-lg shadow w-full dark:bg-gray-800 dark:border-gray-700'>
                        {searchResult?.length ?
                            ...
                            <span className='flex text-sm px-4 py-4 text-gray-700 dark:text-gray-200'>not found</span>
                        }
                    </div>
                }
            </div>
